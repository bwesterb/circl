...
// Writes p whose coefficients are in norm less than γ1 into buf
...
func (p *Poly) PackLeGamma1(buf []byte) {
	...
	for i := 0; i < PolyLeGamma1Size; i += 5 {
		// Coefficients are in [0, γ1) ∪ (Q-γ1, Q)
		p0 := Gamma1 - 1 - p[j]         // ... in [0, γ1) ∪ [γ1-1-Q, 2(γ1-1)-Q]
		p0 += uint32(int32(p0)>>31) & Q // ... in [0, 2(γ1-1)]
		p1 := Gamma1 - 1 - p[j+1]
...
// p will be normalized.
//
// Beware, for arbitrary buf the coefficients of p might exceed γ1.
